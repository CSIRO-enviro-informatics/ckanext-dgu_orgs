'''Routines for matching org names, expressed in different ways.'''

import re
log = __import__('logging').getLogger(__name__)

class OrganisationMatcher:
    external_organisations = {} # canonical_name:external_id
    ...
    def add_external_organisation(self, external_id, *external_names):
        for name in external_names:
            if not name.strip():
                continue
            canonical_name = self.canonical_name(name)
            if canonical_name in self.external_organisations and \
                   self.external_organisations[canonical_name] != external_id:
                log.warning('Duplicate canonical name: %s (%s AND %s)',
                            canonical_name, external_id,
                            self.external_organisations[canonical_name])
                continue
            self.external_organisations[canonical_name] = external_id

    def match_to_external_organisation(self, name):
        canonical_name = self.canonical_name(name)
        return self.external_organisations.get(canonical_name)
