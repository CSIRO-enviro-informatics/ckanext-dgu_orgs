...
from ckanext.dgu_orgs.lib import formats
...
def organisation_hierarchy():
    ...
    out = [ _organisation_hierarchy_recur(node) for node in top_nodes ]
    return out

def _organisation_hierarchy_recur(node):
    title = node['title']
    name  = node['name']
    children = [ _organisation_hierarchy_recur(child) for child in (node['children'] or []) ]
    return {
            'id': node['id'],
            'title':title,
            'name':name,
...
def organisation_hierarchy_mini(group_name_or_id):
    '''Returns HTML for a hierarchy of SOME organisations - the ones which
    are under the same top-level organisation as the given one.'''
    from ckan.logic import get_action
    from ckan import model
    context = {'model': model, 'session': model.Session}
    my_root_node = get_action('group_tree_section')(context=context,
            data_dict={'id': group_name_or_id, 'type': 'organization'})
    return _organisation_hierarchy_recur(my_root_node)

def organisation_abbreviations():
    from ckan import model
...
def is_wms(resource):
    from ckanext.dgu_orgs.lib.helpers import get_resource_wms
...
def user_link_info(user_name, organization=None):  # Overwrite h.linked_user
    ...
    if (is_an_official() or this_is_me or type_ is None):
        ...
        # Joe public just gets a link to the user's organisation(s)
        if user:
            ...
            if type_ == 'official' and is_sysadmin(user):
                ...
                return (matched_group.title,
                       '/organisation/%s' % matched_group.name)
            elif type_ == 'system':
...
def ga_download_tracking_data(resource, pkg_dict, organisation_name, action='download'):
...
def get_ga_custom_dimensions():
    ...
    if c.userobj:
        if c.userobj.sysadmin:
            ...
            if organizations:
                info['dimension2'] = 'organisation' # user_status
    ...
    # user_organisations User  dimension3
    # content_organisation   Hit dimension4
...
def package_organisation_dict(package):
    if not package:
        return {'name':'', 'title': ''}
...
def get_resource_fields(resource, pkg_extras):
    from ckan.lib.base import h
    from ckanext.dgu_orgs.lib.resource_helpers import ResourceFieldNames, DisplayableFields
...
def get_package_fields(package, package_dict, pkg_extras, dataset_was_harvested,
    ...
    from ckanext.dgu_orgs.schema import GeoCoverageType
    from ckanext.dgu_orgs.lib.resource_helpers import DatasetFieldNames, DisplayableFields
...
def group_category(group_extras):
    ...
    if category:
        from ckanext.dgu_orgs.forms.validators import categories
...
def advanced_search_url():
    ...
    if not 'organisation' in params:
        params['parent_organisations'] = c.group.name
...
def get_contact_details(pkg, pkg_extras):
    organisation = c.pkg.get_organization()
    ...
    # If package has no contact details then inherit from the organisation
    if not (name or email or phone) and organisation:
        extras = organisation.extras
...
def get_contact_name(pkg, extras):
    name = extras.get('contact-name')
    if not name:
        organisation = pkg.get_organization()
        if organisation:
            name = organisation.extras.get('contact-name')
    return name
...
def get_foi_contact_name(pkg, extras):
    ...
    if not name:
        organisation = pkg.get_organization()
        if organisation:
            name = organisation.extras.get('foi-name')
    return name
...
def get_foi_contact_details(pkg, pkg_extras):
    ...
    # If package has no FOI contact details then inherit from the organisation
    if not (foi_phone or foi_email or foi_phone or foi_web) and organisation:
        extras = organisation.extras
...
def is_sysadmin_by_context(context):
    ...
    return authz.is_sysadmin(context['user'])
...
def hidden_extra_fields(data):
    from ckanext.dgu_orgs.forms.dataset_form import DatasetForm
...
def edit_organisation_group_name(data):
    ...
    if group_id:
      groups = [p['name'] for p in c.organisations.values() if p['id'] == group_id ]
      group_name = groups[0] if groups else ''

    return group_name
    #return c.organisations.get(group_name, {}) if group_id else data

def edit_organisation_group(data):
    if not data.get('organization'):
        group_name = None
        group_id = None
    else:
...
def organisation_performance_data(organisation, include_sub_organisations):
    """
        Returns additional info for organisations, as traffic lights so that
        it can be viewed on the organisation read page.
    ...
    try:
        ...
        return None
    import time
    #from ckanext.qa.reports import broken_resource_links_for_organisation #TODO ASH
    from ckanext.dgu_orgs.lib import organisation as orglib

    start_time = time.time()

    rcount = orglib.resource_count(organisation, include_sub_organisations)
    log.debug("{o} has {r} resources".format(o=organisation.name, r=rcount))
    ...
    if 'issues' in config['ckan.plugins']:
        ...
        more_than_month = util.old_unresolved(organisation, days=30)
        more_than_fortnight = util.old_unresolved(organisation, days=14)
    ...
    spending = 'green'
    if organisation_has_spend_data(organisation):
        spending = 'red'

    # TODO: Add a count to result of broken_resource_links_for_organisation or write a version
    # that returns count().

    #data = broken_resource_links_for_organisation(organisation.name, include_sub_organisations, use_cache=True)
    #TODO ASH
    data = {"data": []}
    ...
    log.debug("{d}% of resources in {p} are broken".format(d=pct, p=organisation.name))
    ...
    openness = ''
    total, counters = orglib.openness_scores(organisation, include_sub_organisations)
    ...
    log.debug("organisation performance data took {d} seconds".format(d=time.time()-start_time))
    return {
...
def organisation_has_spend_data(organisation):
    return organisation.extras.get('category','') == 'ministerial-department'
...
def search_facets_unselected(facet_keys,sort_by='count'):
    ...
    # Special case behaviour for organisations
    if 'organisation' in facet_keys and request.params.get('organisation',''):
        params_to_keep = dict(facet_params_to_keep())
        del params_to_keep['organisation']
        link = dgu_drill_down_url(params_to_keep.items(), {'parent_organisations':request.params.get('organisation','')})
        unselected.append( (link,'Include sub-organisations',None) )
    return unselected
...
def search_facets_selected(facet_keys):
    ...
    # Special case behaviour for organisations
    if 'parent_organisations' in facet_keys and request.params.get('parent_organisations',''):
        params_to_keep = dict(facet_params_to_keep())
        del params_to_keep['parent_organisations']
        link = dgu_drill_down_url(params_to_keep.items(), {'organisation':request.params.get('parent_organisations','')})
        selected.append( (link,'Include sub-organisations',None) )
    return selected
...
def search_facet_text(key,value):
    ...
    if key=='organisation' or key=='parent_organisations':
...
def ckan_asset_timestamp():
    from ckanext.dgu_orgs.theme.timestamp import asset_build_timestamp
...
def themes():
    from ckanext.dgu_orgs.lib.theme import Themes
...
def closed_organisation_ids():
    """
    Returns the IDs of all of the closed organisations. For use in
    organisation_index.
    """
    import ckan.model as model
...
def all_organisations(exclude=None):
    import ckan.model as model
    orgs = model.Session.query(model.Group.name, model.Group.title)\
        .filter(model.Group.state=='active')\
        .filter(model.Group.type=='organization')

    return orgs.order_by(model.Group.title)

def get_closed_organisation_message(pub):
    """ For organisations that are closed, this function will return a message to
        display on the organisation_read/organisation_edit pages """
    import ckan.model as model
    ...
    if replaced_by:
        ...
        for p in replaced_by:
            g = model.Group.get(p)
            if not g:
                log.warning('Could not find %s which replaces organisation %s', p,
                            pub['name'])
                continue
            organisation_urls.append('<a href="/organisation/%s">%s</a>' % (g.name, g.title,))
        extra_msg = ' or '.join(organisation_urls)
        extra_msg = 'Please see %s instead.' % extra_msg

    return 'This organisation has closed. %s ' % (extra_msg or '')

def put_closed_organisations_last(org_list, closed_organisation_ids):
    open_orgs = []
    closed_orgs = []
    for org in org_list:
        if org['id'] in closed_organisation_ids:
            closed_orgs.append(org)
        else:
            open_orgs.append(org)
    return open_orgs + closed_orgs
...
def get_dgu_dataset_form_options(field_name):
    ...
    '''
    from ckanext.dgu_orgs.forms import dataset_form
...
def get_sla():
    return 'The data organisation commits to the continuing publication of the data and to provide advance notice (on the data.gov.uk dataset page) of any changes to the structure of the data, the update schedule or cessation.'
    ...
    if licence_regexes is None:
        ...
        licence_regexes['ogl-detritus'] = re.compile(
            r'(%s)' % '|'.join((
                ...
                'citation of organisation and online resource required on reuse',
                'conditions',
