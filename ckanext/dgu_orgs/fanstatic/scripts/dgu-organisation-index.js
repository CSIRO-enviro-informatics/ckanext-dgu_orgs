...
(function ($) {
  function buildIndex(parentDiv) {
    var out = [];
    parentDiv.find('> .organisation').each(function (i, row) {
      row = $(row);
      var link = row.find('> .organisation-row > a:first');
      out.push({
        div: row,
        link: link,
        rawtext: link.text(),
  ...
  var container = $('.organisation-hierarchy');
  ...
  if (!container.length) {
    console.error("No .organistation-hierarchy tree!");
  }
  if (!searchBox.length) {
    console.error("No .js-search box!");
  }
  ...
  function updateSearch(searchString) {
    ...
    if (searchString.length == 0 || searchString == 'start typing a name...') {
      ...
      resultCountFooterBox.text('Organisations');
      return;
    }
  ...
  $('.js-expand,.js-collapse').click(function (e) {
    ...
    target.parent().toggleClass('expanded', expanding).toggleClass('collapsed', !expanding);
    return false;
  });
})(jQuery);