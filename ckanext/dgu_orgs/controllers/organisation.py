...
from ckanext.dgu_orgs.plugins_toolkit import c, request, render, _, ObjectNotFound, NotAuthorized, ValidationError, check_access, get_action
from ckan.lib.base import model, h
...
from ckanext.dgu_orgs.lib.organisation import find_group_admins


#from ckanext.dgu_orgs.authentication.drupal_auth import DrupalUserMapping
#from ckanext.dgu_orgs.drupalclient import DrupalClient
#from ckanext.dgu_orgs.plugin import DrupalAuthPlugin

from ckanext.dgu_orgs.forms.validators import categories


from ckan.plugins import PluginImplementations, IMiddleware
from ckanext.dgu_orgs.lib import helpers as dgu_helpers
...
class OrganisationController(OrganizationController):
    ...
    def _send_application( self, group, reason  ):
        ...
        if not recipients:
            if not config.get('dgu.admin.email'):
                log.error('User "%s" prevented from applying for organisation access for "%s" '
        ...
        url = urljoin(g.site_url,
            h.url_for(controller='ckanext.dgu_orgs.controllers.organisation:OrganisationController',
                      action='users', id=group.name))

        log.debug('User "%s" requested organisation access for "%s" which was sent to admin %s (%r) with reason: %r',
        ...
        try:
            for (name,recipient) in recipients:
                mail_recipient(name,
                               recipient,
                               subject='DGUKOrganisationRequest: Please add me as a data.gov.uk organisation',
                               body=email_msg)
        except Exception, e:
            ...
            errors = {"reason": ["data.gov.uk error"]}
            log.error('User "%s" prevented from applying for organisation access for "%s" because of mail configuration error: %s',
                      c.user, group.name, e)
            return self.apply(group.id, errors=errors,
                              error_summary=error_summary(errors))

        h.flash_success('Your application has been submitted to administrator for: %s. If you do not hear back in a couple of days then <a href="http://data.gov.uk/contact">contact the data.gov.uk team</a>' % recipient_organisation, allow_html=True)
    ...
    def apply(self, id=None, data=None, errors=None, error_summary=None):
        ...
        if id:
            ...
            if 'save' in request.params and not errors:
                ...
                if model.Session.query(OrganisationRequest).filter_by(user_name=c.user, group_name=id).all():
                    h.flash_error('A request for this organisation is already in the system. If you have waited more than a couple of days then <a href="http://data.gov.uk/contact">contact the data.gov.uk team</a>', allow_html=True)
                    h.redirect_to('organisation_apply', id=id)
                    return
                else:
                    req = OrganisationRequest(user_name=c.user, group_name=id, reason=reason)
    ...
    def read(self, id, limit=20):
        ...
        context = {'model': model, 'session': model.Session,
                   'user': c.user or c.author,
                   'schema': self._form_to_db_schema(group_type=group_type), #TODO ASH, maybe this should be _db_to_form_schema()
                   'for_view': True}
        ...
        def pager_url(q=None, page=None):
            url = h.url_for(controller='ckanext.dgu_orgs.controllers.organisation:OrganisationController', action='read', id=c.group_dict.get('name'))
            params = [('page', str(page))]
            return url + u'?' + urlencode(params)

        try:
            # Search within group
            fq = ' organisation: "%s"' % c.group_dict.get('name')
            data_dict = {
                'fq':fq,
                'rows':limit,
                'start':(page-1)*limit,
        ...
        c.administrators = self._action('member_list')(
            context, {'id': id, 'object_type': 'user', 'capacity': 'admin'})
        c.editors = self._action('member_list')(
            context, {'id': id, 'object_type': 'user', 'capacity': 'editor'})

        parent_groups = c.group.get_parent_groups(type='organization')
        c.parent_organisation = parent_groups[0] if len(parent_groups) > 0 else None
        ...
        return render('organisation/read.html')
    ...
    def new(self, data=None, errors=None, error_summary=None):
        ...
        return super(OrganisationController, self).new(data, errors, error_summary)

    def organisation_request(self, token, decision=None):
        from ckan import model
        from ckanext.dgu_orgs.model.organisation_request import OrganisationRequest

        try:
            c.req = model.Session.query(OrganisationRequest).filter_by(login_token=token).one()
        ...
        if decision:
            if decision not in ['reject', 'accept']:
                ...
                msg = 'The application of <strong>%s</strong> to the organisation <strong>%s</strong> was marked as rejected' % (c.req_user.fullname, c.req_group.title)
                ...
                msg = '<strong>%s</strong> was added to the organisation <strong>%s</strong>' % (c.req_user.fullname, c.req_group.title)
            model.Session.commit()
    ...
    def organisation_requests(self):
        ...
        for req in model.Session.query(OrganisationRequest).order_by(OrganisationRequest.date_of_request.desc()).all():
            item = {}
    ...
    def _group_form(self, group_type=None):
        return 'organisation/edit_form.html'

    def _new_template(self, group_type):
        return 'organisation/new.html'

    def _about_template(self, group_type):
        return 'organisation/about.html'

    def _index_template(self, group_type):
        return 'organisation/index.html'

    def _admins_template(self, group_type):
        return 'organisation/admins.html'

    def _read_template(self, group_type):
        return 'organisation/read.html'

    def _edit_template(self, group_type):
        return 'organisation/edit.html'
