...
from ckanext.dgu_orgs.lib.helpers import resource_type as categorise_resource
...
def drop_if_same_as_organisation(key, data, errors, context):
    """
    Validates the contact- and foi- data.

    If it's the same as that on the organisation, then the data is dropped,
...
def populate_from_organisation_if_missing(key, data, errors, context):
    """
    If the data is missing, then populate from the organisation.
    """
    from ckan.model.group import Group
...
def validate_organisation_category(key, data, errors, context):
    '''
